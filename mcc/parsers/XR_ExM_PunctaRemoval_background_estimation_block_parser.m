function [done_flag] = XR_ExM_PunctaRemoval_background_estimation_block_parser(batchInds, zarrFullpath, resultFullname, BatchBBoxes, varargin)
...
XR_ExM_PunctaRemoval_background_estimation_block(batchInds, zarrFullpath, resultFullname, ...
    BatchBBoxes, Overwrite=Overwrite, Sigma=Sigma, OTSUMaxPer=OTSUMaxPer, MinThreshold=MinThreshold, ...
    MaxThreshold=MaxThreshold, BaseThreshold=BaseThreshold, volThrsh=volThrsh, ...
    offset=offset, localWinSize=localWinSize, SigmaThrsh=SigmaThrsh, intThrsh=intThrsh, ...
    initDetect=initDetect, detVolThrsh=detVolThrsh, uuid=uuid, debug=debug);
