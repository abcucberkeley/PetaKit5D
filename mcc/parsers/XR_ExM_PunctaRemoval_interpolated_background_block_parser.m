function [done_flag] = XR_ExM_PunctaRemoval_interpolated_background_block_parser(batchInds, zarrFullpath, outFullpath, ...
...
XR_ExM_PunctaRemoval_interpolated_background_block(batchInds, zarrFullpath, ...
    outFullpath, bgFullpath, flagFullname, BatchBBoxes, RegionBBoxes, localBBoxes, ...
    Overwrite=Overwrite, bgFactor=bgFactor, Sigma=Sigma, OTSUMaxPer=OTSUMaxPer, ...
    MinThreshold=MinThreshold, MaxThreshold=MaxThreshold, BaseThreshold=BaseThreshold, ...
    volThrsh=volThrsh, offset=offset, localWinSize=localWinSize, SigmaThrsh=SigmaThrsh, ...
    intThrsh=intThrsh, initDetect=initDetect, detVolThrsh=detVolThrsh, uuid=uuid, ...
    debug=debug);
