function [level, varargout] = XR_thresholdOtsu(imageIn,varargin)
...
% Xiongtao Ruan, 09/2019 improve for efficiency
...
% nzInd = imageIn ~= 0;
nzVals = imageIn(imageIn ~= 0);

%get minumum and maximum pixel values in image
minSignal = min(nzVals);
maxSignal = max(nzVals);

%normalize nonzero value between 0 and 1
imageInNorm = zeros(size(imageIn));
imageInNorm(1 : numel(nzVals)) = (nzVals - minSignal) / (maxSignal - minSignal);
