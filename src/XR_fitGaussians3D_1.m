...
function pStruct = XR_fitGaussians3D_1(vol, X, A, sigma, c, varargin)
...
kLevel = norminv(1-ip.Results.Alpha, 0, 1); % ~2 std above background
...
for p = 1:np
    ...
    if npx >= 20 % only perform fit if window contains sufficient data points
        ...
        % fit
        % tic
        switch ip.Results.FitGaussianMethod
            case 'original'
        ...
        if false
            ...
            % tic
            % [prm_1, prmStd_1, ~, res_1] = fitGaussian3D_test_type_2_refine(window, x_mat, y_mat, z_mat, [X(p,1)-xi(p)+ox X(p,2)-yi(p)+oy X(p,3)-zi(p)+oz A(p) sigma(p,:) c(p)], mode);
            [prm_1, prmStd_1, ~, res_1] = fitGaussian3D_test_type_2_refine(window, x_mat, y_mat, z_mat, [X(p,1)-xi(p)+ox X(p,2)-yi(p)+oy X(p,3)-zi(p)+oz A(p) sigma(p,:) c(p)], mode, 9);
            % toc;
            % tic
            [prm_2, prmStd_2, ~, res_2] = fitGaussian3D_test_type_3(window, x_mat, y_mat, z_mat, [X(p,1)-xi(p)+ox X(p,2)-yi(p)+oy X(p,3)-zi(p)+oz A(p) sigma(p,:) c(p)], mode);
            % toc
            
            res_mat(p, :) = [res.RSS - res_1.RSS, res_1.RSS - res_2.RSS, res_1.RSS - res_2.RSS];
