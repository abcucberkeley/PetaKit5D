...
% 
% 
% Xiongtao Ruan 10/23/2019 improve efficiency
...
% xruan 10/23/2019 first initalize costMatTmp
costMatTmp = zeros(size(propagatedPos, 1),  size(coord2, 1), numSchemes);
...
for iScheme = 1 : numSchemes
    ...
    % xruan 10/23/2019 use matlab function, which is faster
    % costMatTmp(:,:,iScheme) = createDistanceMatrix(coord1,coord2);
    costMatTmp(:,:,iScheme) = pdist2(coord1,coord2);
end
...
% xruan 10/24/2019 use sparse matrix to store the costMat, and first check
% if there is any element is 0, if so,  convert to a tiny positive number.
zeroIdx = costMat == 0;
if any(zeroIdx, 'all')
    costMat(zeroIdx) = 1e-10;
end
costMat = sparse(costMat .* (costMat <= searchRadius));
...
deathBlock = sparse(1 : numFeaturesFrame1, 1 : numFeaturesFrame1, maxCost * ones(numFeaturesFrame1,1));
birthBlock = sparse(1 : numFeaturesFrame2, 1 : numFeaturesFrame2, maxCost * ones(numFeaturesFrame2,1));
...
nonlinkMarker = min(floor(min(costMat(:)))-5,-5);
...
costMat = costMat + (costMat == 0) * nonlinkMarker;
