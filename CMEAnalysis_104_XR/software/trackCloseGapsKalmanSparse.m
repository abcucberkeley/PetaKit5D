...
for iFrame = 1 : numFramesEff
    ...
    notNanIdx = ~isnan(nnDistLinkedFeat(:,iFrame)) & ~any(isnan(coordFrame), 2);    
    % coordFrame = coordFrame(~any(isnan(coordFrame), 2) & notNanIdx,:);
    coordFrame = coordFrame(notNanIdx,:);
    ...
    if ~isempty(coordFrame)
        ...
        % 
        % end
        if size(coordFrame, 1) == 1
            nnDist = 1000;
        else
            [~, nnDist] = knnsearch(coordFrame,coordFrame, 'k', 2);
            nnDist = nnDist(:, 2);            
        end
        
        %store nearest neighbor distances in matrix
        % nnDistLinkedFeat(~isnan(nnDistLinkedFeat(:,iFrame)),iFrame) = nnDist;
        nnDistLinkedFeat(notNanIdx, iFrame) = nnDist;
