...
if largeFile
    ...
    for f = 1 : nF
        funcStrs{f} = sprintf(['XR_MIP_zarr(''%s'',''resultDirName'',''%s'',''axis'',%s,', ...
...
funcStrs_func = @(zarrFullpath_i, zarrFullpaths_j_str, outputFullpath_i, ...
    ...
    '[%s],[%s],[%s],[%s],%s)'], zarrFullpath_i, zarrFullpaths_j_str, outputFullpath_i, ...
    strrep(mat2str(pair_indices), ' ', ','), strrep(mat2str(cuboid_mat_i), ' ', ','), ...
    strrep(mat2str(cuboid_mat_j), ' ', ','), strrep(mat2str(cuboid_overlap_ij_mat), ' ', ','), ...
    param_str);
...
if largeFile
    ...
    for f = 1 : numel(ti)
        ...
        funcStrs{f} = funcStrs_func(zarrFullpaths{f}, zarrFullpaths_j_str, ...
            outputFullpaths{f}, pair_indices, cuboid_mat(f, :), cuboid_mat(tj_f, :), ...
            cuboid_overlap_ij_mat_f);
    end
