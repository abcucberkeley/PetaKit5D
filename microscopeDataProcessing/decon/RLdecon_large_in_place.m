...
for i = 1 : numTasks
    ...
    funcStrs{i} = sprintf(['RLdecon_for_zarr_block([%s],''%s'',''%s'',''%s'',''%s'',%s,%s,', ...
        ...
        '''Background'',%0.20d,''dzPSF'',%0.20d,''DeconIter'',%d,''RLMethod'',''%s'',''wienerAlpha'',%0.20d,', ...
        '''skewed'',[%s],''fixIter'',%s,''scaleFactor'',%d,''useGPU'',%s,''deconMaskFns'',%s,''uuid'',''%s'',', ...
        '''debug'',%s,''psfGen'',%s)'], ...
        ...
        dz, string(Save16bit), string(Overwrite), SkewAngle, string(flipZstack), Background, dzPSF, DeconIter, RLMethod, ...
        wienerAlpha, string(skewed), string(fixIter), scaleFactor, string(useGPU), deconMaskFns_str, uuid, ...
        string(debug), string(psfGen));
end
