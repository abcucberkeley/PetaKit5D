function [] = XR_deskewRotateFrame_old(framePath, xyPixelSize, dz, varargin)
...
% xruan (10/31/2022): copy from commit 80974f6d108294351a7a877d470297b6a60eb4ad for benchmark comparison
% change reader and writer as old methods, change as single for processing
...
if ~exist(dsFullname, 'file') || ip.Results.Overwrite
    frame = single(readtiff_matlab(framePath));
    % flat field correction
    if LLFFCorrection
        LSIm = readtiff_matlab(LSImage);
        BKIm = readtiff_matlab(BackgroundImage);
    ...
    if ip.Results.Save16bit
        writetiff(uint16(ds), dsTempname, 'Mode', 'libtiff');
    else
        writetiff(single(ds), dsTempname, 'Mode', 'libtiff');
    end
...
if ip.Results.Rotate 
    ...
    if ~exist(dsrFullname, 'file')
        ...
        if ~exist('ds', 'var')
            ds = single(readtiff_matlab(dsFullname));
        end
        ...
        if SaveMIP
            ...
            writetiff(uint16(max(dsr, [], 3)), dsrMIPname, 'Mode', 'libtiff');
        end
...
%         end
        ...
        if ip.Results.RescaleRotate
            ...
            writetiff(uint16(dsr), dsrTempName, 'Mode', 'libtiff');
        end
