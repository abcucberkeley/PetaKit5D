...
// skewed space interpolation using simd for weighted sum calculation
...
void copy_uint16_to_float_sequential(const uint16_t* src, float* dst, size_t len) {
...
void copy_uint16_to_float(const uint16_t* src, float* dst, size_t len) {

    #pragma parallel for
    for (size_t i = 0; i + 16 <= len; i += 16) {
        // Load 16 uint16_t values (two 128-bit chunks)
        __m256i src_chunk = _mm256_loadu_si256((__m256i*)&src[i]);

        // Convert uint16_t to uint32_t and then to float
        __m256i src_low  = _mm256_cvtepu16_epi32(_mm256_castsi256_si128(src_chunk));
        __m256i src_high = _mm256_cvtepu16_epi32(_mm256_extracti128_si256(src_chunk, 1));

        __m256 src_float_low  = _mm256_cvtepi32_ps(src_low);
        __m256 src_float_high = _mm256_cvtepi32_ps(src_high);

        // Store the result
        _mm256_storeu_ps(&dst[i], src_float_low);
        _mm256_storeu_ps(&dst[i + 8], src_float_high);
    }
...
void skewed_space_interp_defined_stepsize_16bit_to_float(const uint16_t* im, float* im_int, const float xstep, const float stepsize, const uint8_t Reverse,
                                                         const uint64_t sz[3], const uint64_t outDim[3]){
    ...
    for(uint64_t z = 0; z < nint; z++){
        ...
        if(z_copy){
            #ifndef __APPLE__
            copy_uint16_to_float(im+zorg*nxy, im_int+zint*nxy, nxy);
            #else
            copy_uint16_to_float_sequential(im+zorg*nxy, im_int+zint*nxy, nxy);
            #endif
            continue;
        }
...
void mexFunction(int nlhs, mxArray *plhs[],
    ...
    if (nrhs != 5) mexErrMsgIdAndTxt("volume_warp:inputError","Number of input arguments must be 5");
    ...
    const uint8_t save16bit = (uint8_t)mxIsLogicalScalarTrue(prhs[4]);
    ...
    if(mDType == mxSINGLE_CLASS){
        ...
        if (save16bit){
            ...
            skewed_space_interp_defined_stepsize_16bit_to_float(im, im_int, xstep, stepsize, Reverse, sz, outDim);
        }
    }
    else{
