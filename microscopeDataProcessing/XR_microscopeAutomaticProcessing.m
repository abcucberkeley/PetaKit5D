    ...
    for f = 1 : nF
        ...
        if ~is_done_flag(f, 1) 
            ...
            func_str = sprintf(['XR_deskewRotateFrame(%s,%.20d,%.20d,''SkewAngle'',%.20d,', ...
                '''ObjectiveScan'',%s,''Reverse'',%s,''LLFFCorrection'',%s,''BKRemoval'',%s,', ...
                '''LowerLimit'',%.20d,''LSImage'',''%s'',''BackgroundImage'',''%s'',''Rotate'',%s,', ...
                '''resample'',[%s],''DSRCombined'',%s,''flipZstack'',%s,''Save16bit'',%s)'], ...
        ...
        if ~is_done_flag(f, 2)
            ...
            func_str = sprintf(['XR_matlab_stitching_wrapper(''%s'',''%s'',''ResultDir'',''%s'',', ...
                ...
                ChannelPatterns_str, string(useExistDSR), axisOrder, resampleType, string(Reverse), ...
                string(parseSettingFile), string(xcorrShift), xcorrMode, BlendMethod, string(zNormalize), ...
                string(onlyFirstTP), strrep(num2str(timepoints, '%d,'), ' ', ''), ...
