function [] = zarrToN5(zarrFullpath, varargin)
% convert zarr to N5
% 
...
ip.addParameter('resultDirStr', 'n5/', @ischar);
...
resultFullpath = sprintf('%s/%s.n5', outPath, fsname);
...
fprintf('Convert zarr file %s to n5 ...\n', zarrFullpath);
...
if exist(outTmpPath, 'dir')
    bim = blockedImage(outTmpPath, 'Adapter', N5Adapter);
...
if ~exist(outTmpPath, 'dir')
    ...
    bim = blockedImage(outTmpPath, outSize, blockSize, init_val, "Adapter", N5Adapter, 'Mode', 'w');             
    bim.Adapter.close();    
end
...
for i = 1 : numTasks
    ...
    funcStrs{i} = sprintf(['setup([],true);zarrToN5Block([%s],''%s'',''%s'',''%s'',[%s],[%s],', ...
        '''Overwrite'',%s)'], strrep(num2str(batchInds, '%d,'), ' ', ''), zarrFullpath, ...
        outTmpPath, zarrFlagFullpath, strrep(mat2str(inBatchBBoxes_i), ' ', ','), ...
        strrep(mat2str(outBatchBBoxes_i), ' ', ','), string(Overwrite));
end
