...
for f = 1 : ng
    ...
    if largeFile    
        ...
        func_strs{f} = sprintf(['XR_unmix_channels_frame(%s,%s,''mode'',''%s'',', ...
            ...
            '''save16bit'',%s,''zarrFile'',%s,''saveZarr'',%s,''blockSize'',%s,''uuid'',''%s'')'], ...
            frameFullpaths_str,  mat2str_comma(unmixFactors, 10), mode, mat2str_comma(unmixSigmas, 10), ...
            resultDirName, channelInd, string(save16bit), string(zarrFile), ...
            string(saveZarr), mat2str_comma(blockSize), uuid);
    end
end
