...
function [lftRes, res] = runLifetimeAnalysis3D(data, varargin)
...
ip.addParamValue('AnalysisPath', 'Analysis', @ischar);
...
ip.addParamValue('Cutoff_f', 3, @isscalar);
...
ip.addParamValue('PoolDatasets', true, @islogical);
ip.addParamValue('ShowStatistics', true, @islogical);
...
[lftData, outlierIdx] = getLifetimeData(data, 'Overwrite', ip.Results.Overwrite,...
    ...
    'RemoveOutliers', ip.Results.RemoveOutliers, 'Mask', false, 'Colormap', ip.Results.Colormap, ...
    'AnalysisPath', ip.Results.AnalysisPath,'ProcessedTracks', ip.Results.ProcessedTracks, 'LifetimeData', ip.Results.LifetimeData,...
...
elseif ~isempty(ip.Results.MaxIntensityThreshold)
    ...
    for i = 1:nd
        ...
        save([data(i).source ip.Results.AnalysisPath filesep 'Parameters.mat'], '-struct', 'prm');
    end
...
for i = 1:nd
    ...
    if ~isempty(selIdx)
        idxMI = idxMI & horzcat(selIdx{:}); %% changed from vertcat to horzcat -- GU
    end
