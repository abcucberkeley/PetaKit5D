...
% xruan 01/02/2020 check if the condDir is absolute path. If not, convert it to full. 
if ~java.io.File(condDir).isAbsolute
    dir_info = dir(condDir);
    if ~isempty(dir_info)
        condDir = dir_info(1).folder;
    end    
end
...
for k = 1:nCells
    ...
    for c = 1:nCh
        ...
        if ~exist(channels{c}, 'dir') && ip.Results.ChannelMatching
            ...
            if any(nonEmpty_inds)
                channels{c} = channelPath{nonEmpty_inds};
            end            
        end           
