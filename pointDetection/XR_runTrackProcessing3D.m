...
%   'TrackerOutput' : tracking file name. 
%        'FileName' : processed track result file name.
%   'DetectionFile' : detection file name.
%      'WindowSize' : window size for detection (filling gaps). 
%       'BufferAll' : buffer tracks. 
%     'ResultsPath' : Result directory to store the results. 
%  'ParallelMethod' : parallel computin method method. 
%                       'parfor' or 'lock_distributed' (default, only available for Mac and Linux). 
...
% edited Gokul - line 242
% xruan 11/06/2019 check if tracks is empty after remove single frames
% xruan 12/19/2019 add option for lock-based distribution 

function XR_runTrackProcessing3D(data, varargin)

ip = inputParser;
ip.CaseSensitive = false;
ip.addRequired('data', @isstruct);
ip.addParameter('Buffer', [3 3], @(x) numel(x)==2);
ip.addParameter('Overwrite', false, @islogical);
ip.addParameter('FitMixtures', false, @islogical); % fairly robust
ip.addParameter('TrackerOutput', 'trackedFeatures.mat', @ischar);
ip.addParameter('FileName', 'ProcessedTracks.mat', @ischar);
ip.addParameter('DetectionFile', 'Detection3D.mat', @ischar);
ip.addParameter('Frames', arrayfun(@(x) 1:x.movieLength, data, 'unif', 0), @(x) numel(unique(diff(x)))==1); %check that frame rate is constant
ip.addParameter('Preprocess', true, @islogical);
ip.addParameter('Postprocess', true, @islogical);
...
WriteAmira = ip.Results.WriteAmira;

% par
switch ip.Results.ParallelMethod
    case 'parfor'
        parfor i = 1:length(data)
            if ~(exist([ip.Results.ResultsPath{i} FileName],'file')==2) || overwrite %#ok<PFBNS>
                data(i) = main(data(i), ip.Results, ResultsPath{i}); %#ok<PFOUS>
            else
    ...
    case 'lock_distributed'
        for i = 1 : numel(data)
            chunk_lock_clean(ResultsPath{i}, 1 * data(i).movieLength);
            ...
            if exist(temp_filename, 'file')
                fprintf('Processing tracks from %s... \n', getShortPath(data(i)));                
                continue;
            else
                fclose(fopen(temp_filename, 'w'));
            end
            
            data(i) = main(data(i), ip.Results, ResultsPath{i}); 
...
% xruan 12/19/2019 
if all(arrayfun(@(ind) ~isfield(frameInfo(ind), 'x') || isempty(frameInfo(ind).x), 1 : numel(frameInfo)))
    fprintf('runTrackProcessing: detections for all frames are empty for %s\n', getShortPath(data));
    return
end
...
if preprocess
    ...
    if nTracks == 0
        fprintf('runTrackProcessing: tracking data is empty for %s\n', getShortPath(data));
        return;
    end
...
if isfield(frameInfo, 'mixtureIndex')
    ...
    'startBuffer', [], 'endBuffer', [], 'MotionAnalysis', [], 'mixtureIndex',[]);
else
tracks(1:nTracks) = struct('t', [], 'f', [],...
    'x', [], 'y', [], 'z', [], 'A', [], 'c', [],...
    'x_pstd', [], 'y_pstd', [], 'z_pstd', [], 'A_pstd', [], 'c_pstd', [],...
...
[~,loc] = ismember({'s','x_init', 'y_init', 'z_init', 'xCoord', 'yCoord', 'zCoord', 'amp', 'dRange'}, mcFieldNames); % added 's'
idx(loc(loc~=0)) = false;
...
%     [tracks(nGaps./trackLengths>=0.5).catIdx] = deal(2);
    ...
    %==========================================
    % Compute displacement statisticsmean
    %==========================================
...
save([resultsPath opts.FileName], 'tracks', 'processingInfo', '-v7.3');

% xruan 12/20/2019 
if opts.WriteAmira
    amiraRootPath = [resultsPath filesep 'Amira'];
    mkdir(amiraRootPath);
    amiraPath = [amiraRootPath filesep 'Tracking'];    
    mkdir(amiraPath);    
    tracks = tracks(ismember([tracks.catIdx], [1:4]));
    
    XR_amiraWriteTracks([amiraPath filesep 'tracking'], tracks, data, 'FlipZ', true);
    % GU_amiraWriteTracks([amiraPath filesep 'tracking'], tracks, data, 'FlipZ', true);
end
...
[x_mat, y_mat, z_mat] = meshgrid(0 : w2x * 2, 0 : w2x * 2, 0 : w2z * 2);
% 
% tic
[prm, prmStd, ~, res] = XR_fitGaussian3D(window, x_mat, y_mat, z_mat, [x-xi+ox y-yi+oy z-zi+oz ai sigma ci], 'xyzAc');
% toc
...
if (dx > -w1x && dx < w1x && dy > -w1x && dy < w1x && dz > -w1z && dz < w1z)
    ...
    % toc
    % tic
    % [prm, prmStd, ~, res] = fitGaussian3D_XR(window, x_mat, y_mat, z_mat, [x-xi+ox y-yi+oy z-zi+oz ai sigma ci], 'Ac');
    % toc
    ps.x = x;
    ps.y = y;
    ps.z = z;
