...
% 
% 
% Xiongtao Ruan, 10/24/2019 improve for efficiency
% XR, 11/05/2019 use while loop to check if rowStart is less than 1.
% XR, 11/09/2019 change all rowStart with while loop
    ...
    for iFrame = 1 : numFrames
        ...
        switch movieInfo(iFrame).num
            ...
            otherwise %if there is more than 1 feature
                ...
                % nnDist = nnDist(:,2);
                [~, nnDist] = knnsearch(movieInfo(iFrame).allCoord(:,1:2:end), movieInfo(iFrame).allCoord(:,1:2:end), 'k', 2);
                nnDist = nnDist(:, 2);                
        end
...
for iFrame = 1 : numFrames-1
    ...
    if numFeaturesFrame1 ~= 0 %if there are features in 1st frame
        ...
        if numFeaturesFrame2 ~= 0 %if there are features in 2nd frame
            ...
            % xruan
            switch costMatName
                case 'costMatLinearMotionLink2'
                    [costMat,propagationScheme,kalmanFilterInfoTmp,nonlinkMarker] ...
                        = costMatLinearMotionLink2(movieInfo,kalmanFilterInfo(iFrame),...
                        costMatParam,nnDistFeatures(1:numFeaturesFrame1,:), ...
                        probDim,prevCostStruct,featLifetime,trackedFeatureIndx,iFrame);
                case 'costMatRandomDirectedSwitchingMotionLink'
                    [costMat,propagationScheme,kalmanFilterInfoTmp,nonlinkMarker] ...
                        = costMatRandomDirectedSwitchingMotionLink(movieInfo,kalmanFilterInfo(iFrame),...
                        costMatParam,nnDistFeatures(1:numFeaturesFrame1,:), ...
                        probDim,prevCostStruct,featLifetime,trackedFeatureIndx,iFrame);
            end
            ...
            if any(costMat(:)~=nonlinkMarker) %if there are potential links
                ...
                % if rowStart <= 1
                while rowStart <= 1
                    % XR 11/06/2019 in some case, numTracksWorstCase is zero 
                    if numTracksWorstCase == 0
                        numTracksWorstCase = max(1, 1 - rowStart);
                    end                    
                    trackedFeatureIndxAux = [zeros(numTracksWorstCase,numFrames); ...
