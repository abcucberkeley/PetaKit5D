...
parfor i = 1:nd
    fpath = [data(i).source ip.Results.AnalysisPath filesep ip.Results.LifetimeData]; %#ok<PFBNS>
...
for i = 1:nd
    [~,~] = mkdir([data(i).source ip.Results.AnalysisPath]);
    fpath = [data(i).source ip.Results.AnalysisPath filesep ip.Results.LifetimeData];
    if ~(exist(fpath, 'file')==2) || ip.Results.Overwrite
