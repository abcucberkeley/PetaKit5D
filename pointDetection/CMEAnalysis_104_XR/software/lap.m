...
% xruan 10/23/2019 do not do the work on the transposed cost matrix, which is very costly!
% do the work on the transposed cost matrix!
% cc = cc';
% find the significant elements. If sparse input, find nonzero elements
if issparse(cc)
    % xruan just as transpose, swap rowIdx and colIdx 
    % [rowIdx, colIdx, val] = find(cc);
    [colIdx, rowIdx, val] = find(cc);
else
    % xruan just as transpose, swap rowIdx and colIdx     
    % [rowIdx, colIdx] = find(cc ~= NONLINK_MARKER);
    % val = cc(cc ~= NONLINK_MARKER);
    linkIdx = cc ~= NONLINK_MARKER;
    [colIdx, rowIdx] = find(linkIdx);
    val = cc(linkIdx);
end

% xruan it seems the input col and row index have to be sorted. 
[colrowIdx, inds] = sortrows([colIdx, rowIdx]);
colIdx = colrowIdx(:, 1);
rowIdx = colrowIdx(:, 2);
val = val(inds);
...
if issparse(cc)
    ...
    if (~all(sum(linkIdx,1)) || ~all(sum(linkIdx,2)))
        error('LAP:BadCostMatrix',...
            'Rows and columns of the cost matrix must allow at least one possible link!')
    end
end
