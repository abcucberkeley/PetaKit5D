function [xmin,xmax,ymin,ymax,zmin,zmax,nn] = XR_subVolumeCoordinatesExtraction_test(volSize, varargin)
...
ip.addParameter('goodNum', true, @isnumeric); % [minimum, maximum] size. 
...
while prod(chosenChunkSize) > maxSubVolume
    ...
    [~, increaseInd] = max(partitionMaxRatios);
...
if goodNum
    for i = 1 : 3
        csz(i) = findGoodFactorNumber_test(csz(i), 1);
    end
end
...
function [goodNum] = findGoodFactorNumber(givenNum, directionFlag)
